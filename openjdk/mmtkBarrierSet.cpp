...
MMTkAllocatorOffsets get_immix_large_tlab_top_and_end_offsets(AllocatorSelector selector) {
  ...
  if (selector.tag == TAG_IMMIX) {
    int allocator_base_offset = allocators_base_offset
      + in_bytes(byte_offset_of(Allocators, immix))
      + selector.index * sizeof(ImmixAllocator);
    tlab_top_offset = allocator_base_offset + in_bytes(byte_offset_of(ImmixAllocator, large_cursor));
    tlab_end_offset = allocator_base_offset + in_bytes(byte_offset_of(ImmixAllocator, large_limit));
  } else {
    fatal("unreachable\n");
    // Setting values to make compiler happy about unitialized variables. This
    // case should never be reached in practice, however.
